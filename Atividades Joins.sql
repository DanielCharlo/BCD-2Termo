...
-- atividade 5
select cliente.nome, count(*) as qtde, sum(venda.valor) as soma_total
from cliente inner join venda 
on cliente.codcli = venda.codcli
group by cliente.nome; 
...
from cliente inner join venda 
on cliente.CODCLI = venda.CODCLI
where year (venda.vencto) < 2003 and month (venda.vencto) < 12 and day (venda.vencto) < 31
group by cliente.nome;

-- atividade 7
select cliente.nome, count(*) as atrasos, sum(venda.valor) as valor_bruto, sum(venda.valor * 1.1) as total_geral
from cliente inner join venda 
on cliente.codcli = venda.codcli
where year (venda.vencto) < 2003 and month (venda.vencto) < 12 and day (venda.vencto) < 31
group by cliente.nome;
